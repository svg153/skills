...
8. **Apply the shared GitHub policy consistently.** For the repository baseline, copy `scripts/configure-github.mjs` to the target's `.github/scripts/` and `assets/repository.example.json` to `.github/repository-bootstrap.json`, then fill in only the repository, description, homepage, and topics. The policy itself is fixed: squash-only merges, issues/discussions, standard labels, vulnerability reporting, and a `main` ruleset requiring PR review, CODEOWNERS, resolved threads, linear history, and the `test`, `title`, and `commits` checks. The authenticated maintainer is the sole bypass actor, with `always` mode: they can push directly and bypass every rule, not only PR reviews/checks. Do not add per-setting opt-outs; change the shared script only when intentionally revising the standard for all repositories.
   - Require Node.js and authenticated GitHub CLI (`gh auth status`); the script delegates all credentials and API calls to `gh` and never handles tokens. If a prerequisite is missing, install it separately using the platform's normal package manager; do not add self-install behavior to the script.
   - Run `node .github/scripts/configure-github.mjs --config .github/repository-bootstrap.json --plan` to inspect the complete read-only plan. Once the user authorizes GitHub changes, use the same command with `--apply`; it applies the complete policy, never a selectable subset. It only manages an existing public repo whose default branch is `main` and requires admin access. It replaces the topic list and upserts the named ruleset and baseline labels; unrelated rulesets/labels are left alone. A failed run may have applied earlier operations; rerun after resolving the reported error.
   - Create or verify the `test`, `title`, and `commits` status checks and `.github/CODEOWNERS` before applying the ruleset, or the required checks/review policy may block merging.
   - Keep GitHub CLI authenticated to the intended account/host. For multiple repositories, prefer organization rulesets or a central settings product only after choosing one source of truth; do not combine overlapping governance systems.
...
## Shared baseline; project-specific metadata
...
- The packaged executor is the common baseline for repository settings and labels. Keep per-project variation in the four metadata fields, not by skipping baseline controls.
- The executor does not create repositories, workflow files, CODEOWNERS, or releases; establish and verify those project files before applying the ruleset.
